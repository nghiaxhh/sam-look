...
import { createDeck, shuffleDeck, dealCards, findPlayerWithLowestCard, sortHand } from '../../src/game/card-utils.js';
import { classifyCombination, canBeat } from '../../src/game/combination-utils.js';
import type { GameStatePayload, GamePlayerInfo, TrickPlayInfo, GameLogEntry as ClientLogEntry, SamResult } from '../../src/types/socket-events.js';
import type { Room, ServerGameState, ServerPlayer, TrickPlay, GameLogEntry, SamResult as ServerSamResult } from './room-manager.js';
...
export class GameManager {
  ...
  startGame(room: Room, previousWinnerId?: string): void {
    ...
    if (previousWinnerId) {
      firstPlayerIndex = room.players.findIndex(p => p.socketId === previousWinnerId);
  ...
  declareSam(room: Room, socketId: string): { success: boolean; error?: string } {
    const gs = room.gameState;
    if (!gs || gs.phase !== 'declaring') return { success: false, error: 'Không thể báo sâm lúc này' };

    const player = gs.players.find(p => p.socketId === socketId);
    if (!player) return { success: false, error: 'Người chơi không tồn tại' };

    if (gs.samPlayerId) return { success: false, error: 'Đã có người báo sâm rồi!' };
  ...
  skipSam(room: Room, socketId: string): { success: boolean; error?: string } {
    const gs = room.gameState;
    if (!gs || gs.phase !== 'declaring') return { success: false, error: 'Không thể bỏ qua lúc này' };
  ...
  private startSamPlay(gs: ServerGameState, samPlayerId: string): void {
    const samPlayerIndex = gs.players.findIndex(p => p.socketId === samPlayerId);
    gs.phase = 'sam_playing';
    gs.currentPlayerIndex = samPlayerIndex;
    gs.currentTrick = null;
    gs.trickLeaderId = samPlayerId;
    gs.players.forEach(p => { p.hasPassed = false; });
  }

  processSamPlay(room: Room, socketId: string, cardIds: string[]): { success: boolean; error?: string } {
    const gs = room.gameState;
    if (!gs || gs.phase !== 'sam_playing') return { success: false, error: 'Không phải lượt sâm' };
    if (gs.samPlayerId !== socketId) return { success: false, error: 'Chỉ người báo sâm mới được đánh' };

    const currentPlayer = gs.players[gs.currentPlayerIndex];
    if (currentPlayer.socketId !== socketId) return { success: false, error: 'Chưa đến lượt bạn' };
    ...
    currentPlayer.hand = sortHand(currentPlayer.hand.filter(c => !cardIdSet.has(c.id)));
    ...
    gs.currentPlayerIndex = this.getNextPlayerIndex(gs, gs.players.findIndex(p => p.socketId === socketId));
    this.resetDeadline(gs);

    return { success: true };
  }

  processSamChallenge(room: Room, socketId: string, cardIds: string[]): { success: boolean; error?: string } {
    const gs = room.gameState;
    if (!gs || gs.phase !== 'sam_playing') return { success: false, error: 'Không phải lượt sâm' };
    if (gs.samPlayerId === socketId) return { success: false, error: 'Người báo sâm không được chặn chính mình' };

    const currentPlayer = gs.players[gs.currentPlayerIndex];
    if (currentPlayer.socketId !== socketId) return { success: false, error: 'Chưa đến lượt bạn' };

    if (!gs.currentTrick) return { success: false, error: 'Không có bài để chặn' };
  ...
  processSamPass(room: Room, socketId: string): { success: boolean; error?: string } {
    const gs = room.gameState;
    if (!gs || gs.phase !== 'sam_playing') return { success: false, error: 'Không phải lượt sâm' };
    if (gs.samPlayerId === socketId) return { success: false, error: 'Người báo sâm không được bỏ lượt' };
    ...
    const samPlayerIndex = gs.players.findIndex(p => p.socketId === gs.samPlayerId);
    ...
    gs.currentPlayerIndex = this.getNextChallengerIndex(gs, gs.currentPlayerIndex);
    this.resetDeadline(gs);
  ...
  private getNextChallengerIndex(gs: ServerGameState, fromIndex: number): number {
    const count = gs.players.length;
  ...
  private samFailed(gs: ServerGameState, blockerId: string, blockerName: string): void {
    const samPlayer = gs.players.find(p => p.socketId === gs.samPlayerId)!;
  ...
  private normalWin(gs: ServerGameState, winnerId: string): void {
    ...
    winner.score += totalCards;

    gs.phase = 'game_over';
    gs.winnerId = winnerId;
  }

  processPlay(room: Room, socketId: string, cardIds: string[]): { success: boolean; error?: string } {
    const gs = room.gameState;
    if (!gs || gs.phase !== 'playing') return { success: false, error: 'Game chưa bắt đầu' };
  ...
  processPass(room: Room, socketId: string): { success: boolean; error?: string } {
    const gs = room.gameState;
    if (!gs || gs.phase !== 'playing') return { success: false, error: 'Game chưa bắt đầu' };
    ...
    if (gs.currentTrick === null) return { success: false, error: 'Đang đánh tự do, không thể bỏ' };
    ...
    const activePlayers = gs.players.filter(p => p.hand.length > 0 && !p.hasPassed);

    if (activePlayers.length <= 1) {
      // Trick leader gets free turn
      const leaderIndex = gs.players.findIndex(p => p.socketId === gs.trickLeaderId);
  ...
  private autoPassDisconnected(room: Room): void {
    ...
    if (!current.isConnected && current.hand.length > 0) {
      ...
      // Play their lowest card automatically
      else if (current.hand.length > 0) {
        const lowestCard = current.hand[0]; // hand is sorted
  ...
  autoTimeoutAction(room: Room): void {
    ...
    if (gs.phase === 'playing') {
      ...
      if (gs.currentTrick !== null) {
        ...
        if (current.hand.length > 0) {
          const lowestCard = current.hand[0];
          this.processPlay(room, current.socketId, [lowestCard.id]);
        }
      }
      return;
    }
    ...
    if (gs.phase === 'sam_playing') {
      ...
      if (current.socketId === gs.samPlayerId) {
        // Sam player's turn → play lowest card
        if (current.hand.length > 0) {
          const lowestCard = current.hand[0];
          this.processSamPlay(room, current.socketId, [lowestCard.id]);
        }
      } else {
